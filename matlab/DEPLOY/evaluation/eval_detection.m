...
function [ap recall precision tp_ret fp_ret num_pos_ret] = eval_detection(predict_file,gtruth_dir,meta_file,...
...
over_file=zeros(length(gt_img_ids),2);
over_counter=0;
    ...
    for j=1:num_obj
        ...
        for k=1:num_gt_obj
            ...
            if iw>0 & ih>0                
                ...
                over_file(over_counter,1)=ov;
                over_file(over_counter,2)=gt_thr(k);
            end
        end
        if kmax > 0
            tp(j) = 1;
...
for c=1:num_classes
    ...
    num_pos_ret(c)=num_pos_per_class(c);
