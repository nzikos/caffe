classdef extraction_model < handle
    %extraction_model is used to obtain paths and extract metadata-objects from a specific contest. In case of a non supported contest, one can create his/her own functions and pass them to the function handlers under CONTEST object.
    ...
    methods
        ...
        function set_objects(model,dims)
            model.objects.dims = dims;
        end
        %% METADATA HANDLERS
        function build_metadata(model)
            model.metadata = model.metadata.build_metadata(model.paths,...
                                                           model.contest,...
                                                           model.sets.set);
        end
        ...
        function print_metadata(model)
            model.metadata.print(model.sets.set,...
                                 model.contest.class_names_map);
        end
        ...
        %% OBJECTS HANDLERS
        function model = build_objects(model)
            model.objects = model.objects.build_objects(model.sets.set,...
                                                        model.metadata,...
                                                        model.paths,...
                                                        model.contest);
        end
        function model = load_objects(model)
