classdef NET_STRUCTURE < handle
    ...
    %     4. Sigmoid
    %     5. Tanh
    %   Output layers (error functions) are:
    %     1. SoftmaxWithLoss
    ...
    %% PRORERTIES
    %
    %   prototxt_path    : The path in which prototxt files will be stored.
    %
    %   train_batch_size : The batch size for training phase.
    %
    %   val_batch_size   : The batch size for validation phase.
    %
    %   test_batch_size  : The batch size for testing/operation phase.
    %
    %   objects_size     : The dimensions of the network's input as HxWxD
    %
    %   valid_structure  : Internal var/flag which states the validity of
    %                      the structure.
    %   in_place         : Internal var/flag which states whether in-place
    %                      trasformations (wherever possible) will be used 
    %                      or not.
    %   layers           : Cell array which contains the layers.
    %
    %   params           : Cell array which contains the initialized
    %                      learnable parameters of the layers.
    %
    %   sub_counter      : Internal layer's counter
    %
    %   m_mult           : Local momentum multiplier for each layer.
    %
    %   lr_mult          : Local learning rate multiplier for each layer.
    %
    %   wd_mult          : Local weight decay multiplier for each layer.
    %
    %% SETTERS
    %
    %   set_batch_size       : Sets the batch for each phase
    %
    %   set_objects_dims     : Sets the input size of the network HxWxD
    %
    %   use_in_place         : Sets the flag in_place
    %
    %   add_layer            : Adds a layer
    %
    %% INTERNAL METHODS
    %
    %   get_bottom_top_blobs : Used to connect the layers in cascading
    %                          topology.
    %
    %   validate_structure   : Validates the structure.
    %
    %% METHODS
    %
    %   create_prototxt      : Creates the prototxt for the asked phase
    %    
    ...
    methods
        ...
        %% SET USE IN PLACE
        function use_in_place(structure,argval)
            if ~(argval==0 || argval==1)
                APP_LOG('last_error','Use boolean arguments in use_in_place()');
            end
            structure.in_place=argval;
        end
        %% ADD LAYER
        function add_layer(structure,type,varargin)
            ...
            switch type
                ...
                case 'Pooling'
                    ...
                    name = ['pool_' num2str(params_counter-1) '_' num2str(structure.sub_counter)];
                    if isempty(varargin)
                        APP_LOG('last_error','%s: Erroneous initialization. Consider reading the documentation',name);
                    end
                case 'MVN'
                    structure.sub_counter   = structure.sub_counter + 1;
                    name = ['MVN_' num2str(params_counter-1) '_' num2str(structure.sub_counter)];                    
                    if isempty(varargin)
                        APP_LOG('last_error','%s: Erroneous initialization. Consider reading the documentation',name);
                    end                    
                case 'LRN'
                    structure.sub_counter   = structure.sub_counter + 1;
                    name = ['LRN_' num2str(params_counter-1) '_' num2str(structure.sub_counter)];
                    if isempty(varargin)
                        APP_LOG('last_error','%s: Erroneous initialization. Consider reading the documentation',name);
                    end                    
                case 'Activation'
                    ...
                    switch( varargin{1}{1} )
                        case 'ReLU'
                            name = ['relu_' num2str(params_counter-1) '_' num2str(structure.sub_counter)];
                        case 'Sigmoid'
                            name = ['sigmoid_' num2str(params_counter-1) '_' num2str(structure.sub_counter)];                    
                        case 'Tanh'
                            name = ['tanh_' num2str(params_counter-1) '_' num2str(structure.sub_counter)];
                        otherwise
                            APP_LOG('last_error','Unknown activation type %s. Use ReLU,Sigmoid,Tanh');
                    end
                case 'Dropout'
                    structure.sub_counter   = structure.sub_counter + 1;
                    name = ['drop_' num2str(params_counter-1) '_' num2str(structure.sub_counter)];
                    if isempty(varargin)
                        APP_LOG('last_error','%s: Erroneous initialization. Consider reading the documentation',name);
                    end                    
                case 'Output'
                    structure.sub_counter   = structure.sub_counter + 1;                    
                    name = ['output_' num2str(params_counter-1) '_' num2str(structure.sub_counter)];
                    if isempty(varargin)
                        APP_LOG('last_error','%s: Erroneous initialization. Consider reading the documentation',name);
                    end                    
                otherwise
            ...
            switch type
                ...
                case 'Activation'
                    if structure.in_place
                        top=bottom;
                    end
                    layer = Activation_layer(bottom,bottom_size,top,name,varargin{1});                    
                case 'Dropout'
                    if structure.in_place
                        top=bottom;
                    end                    
                    layer = Dropout_layer(bottom,bottom_size,top,name,varargin{1});
                case 'Output'
