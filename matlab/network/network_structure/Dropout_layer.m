            ...
            switch varargin{2}
                case 'train'
                    ...
                    if ~isempty(varargin{1})
                        fprintf(varargin{1},formatSpec,[obj.name '_1'],obj.top,obj.top,obj.dropout_ratio);
                    else
                        APP_LOG('warning','No filepath to print prototxt supplied');
                    end
                case 'validation'
                    %CAFFE IS ALREADY SCALING NEURONS OUTPUTS THROUGH
                    %TRAINING IN ORDER NOT TO USE DROPOUT WHILE ON TESTING
                    %PHASE. We can do this override because Dropout layers
                    %are declared as in-place transformations under
                    %NET_STRUCTURE.
                case 'test'
                    %CAFFE IS ALREADY SCALING NEURONS OUTPUTS THROUGH
                    %TRAINING IN ORDER NOT TO USE DROPOUT WHILE ON TESTING
...
%                 APP_LOG('debug',formatSpec,obj.name,obj.top,obj.top,obj.num_output);
        end
    end    
end
