...
void MVNLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,const vector<Blob<Dtype>*>& top) {
	...
	if (this->layer_param_.mvn_param().normalize_variance()) {
		...
		caffe_gpu_powx(bottom[0]->count(), bottom_data, Dtype(2),temp_.mutable_gpu_data());
		...
		// subtract mean
		caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num, dim, 1, -1., mean_.gpu_data(), sum_multiplier_.gpu_data(), 0., temp_.mutable_gpu_data());
...
void MVNLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top, const vector<bool>& propagate_down, const vector<Blob<Dtype>*>& bottom) {
	...
	if (this->layer_param_.mvn_param().normalize_variance()) {
		...
		// put the squares of bottom into temp_ (POSSIBLE LEFTOVER... gone unnoticed)
		//caffe_gpu_powx(temp_.count(), bottom_data, Dtype(2), temp_.mutable_gpu_data()); //"WHY? Looks like you are rewritting temp_.cpu_data" 
		...
		caffe_gpu_gemv<Dtype>(CblasNoTrans, num, dim, 1. / dim, top_diff, sum_multiplier_.gpu_data(), 0., mean_.mutable_gpu_diff());
		caffe_gpu_gemm<Dtype>(CblasNoTrans, CblasNoTrans, num, dim, 1, -1., mean_.gpu_diff(), sum_multiplier_.gpu_data(), 0., temp_.mutable_gpu_diff());
